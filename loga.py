...
# Conexão SSH
try:
    print("[DEBUG] Iniciando conexão SSH com o roteador...")
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    print(f"[DEBUG] Conectando ao roteador {router_ip} como usuário {router_user}...")
    ...
    time.sleep(5)  # Aguarda a execução do script
    ...
    print("[DEBUG] Fechando conexão SSH...")
    ssh.close()
    print("[DEBUG] Conexão SSH fechada.")

    # Envia e-mail de confirmação
    print("[DEBUG] Preparando para enviar e-mail de confirmação...")
    enviar_email(
        "Relatório de Execução",
        f"O comando bbe-smgd.sh foi executado no roteador {router_ip}."
    )

except paramiko.AuthenticationException:
    print("[ERRO] Falha na autenticação SSH. Verifique o usuário e senha.")
except paramiko.SSHException as e:
    print(f"[ERRO] Erro na conexão SSH: {e}")
except Exception as e:
